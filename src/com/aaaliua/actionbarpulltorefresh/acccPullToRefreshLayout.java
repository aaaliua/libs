package com.aaaliua.actionbarpulltorefresh;
 ...
 */
@SuppressLint("NewApi")
public class acccPullToRefreshLayout extends FrameLayout {
	...
	private static final int SHOW_CHECKING_FOR_MAIL_DURATION_IN_MILLIS = 1 * 1000; // 1
																					// seconds
	...
	// Max number of times we display the same sync turned off warning message
	// in a toast.
	// After we reach this max, and device/account still has sync off, we assume
	// user has
	...
	public acccPullToRefreshLayout(Context context) {
		this(context, null);
	}

	public acccPullToRefreshLayout(Context context, AttributeSet attrs) {
		this(context, attrs, -1);
	}

	public acccPullToRefreshLayout(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.mActivity = (Activity) context;
		mWindowManager = (WindowManager) context
				.getSystemService(Context.WINDOW_SERVICE);
		mHintText = new acccPullToRefreshLayout.HintText(context);
	...
	// protected void setConversationContext(ConversationListContext
	// convListContext) {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		// Delayed to this step because activity has to be running in order for
		// view to be
		...
		case MotionEvent.ACTION_DOWN:
			...
			// ((ConversationListFragment)mOnRefreshListener).mConversationAdapter
			// != null &&
			// ((ConversationListFragment)mOnRefreshListener).mConversationAdapter.getCheckedItemCount()
			// > 0) {
			// break;
			// }
			...
			// }
			// 先判断是否有子view 在判断子view是属于哪种类型
			if (this.getChildAt(0) != null) {
				if (this.getChildAt(0) instanceof RecyclerView) {
					RecyclerView view = (RecyclerView) getChildAt(0);
				...
				} else if (this.getChildAt(0) instanceof AbsListView) {
					AbsListView view = (AbsListView) getChildAt(0);
					if (view.getChildAt(0) == null) {
						startMovementTracking(y);
					} else {
						if (view.getChildAt(0).getTop() == 0) {
				...
				} else if (this.getChildAt(0) instanceof ScrollView) {
					ScrollView view = (ScrollView) getChildAt(0);
					if (view.getChildAt(0) == null) {
						startMovementTracking(y);
					} else {
						if (view.getChildAt(0).getTop() == 0) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mTrackingScrollMovement) {
				...
				// LogUtils.d(LOG_TAG,
				// "ignoring swipe to refresh on DRAFT folder");
				// break;
				// }
				// if
				// (mActivity.getFolderController().getFolder().supportsCapability(
				// FolderCapabilities.IS_VIRTUAL)) {
				// // Don't allow refreshing of virtual folders.
				// LogUtils.d(LOG_TAG,
				// "ignoring swipe to refresh on virtual folder");
				// break;
				// }
				...
				float verticalDistanceFromMaxDp = verticalDistanceFromMaxPx
						/ mDensity;
				...
				// Otherwise hint how much further user needs to drag to trigger
				// sync by
				// expanding the sync status bar proportional to how far they
				// have dragged.
				...
				setTriggerScale(mAccelerateInterpolator
						.getInterpolation(verticalDistanceDp
	...
	private void cancelMovementTracking() {
		if (mTrackingScrollMovement) {
			// Shrink the status bar when user lifts finger and no sync has
			// happened yet
			if (mSyncTriggerBar != null) {
				mSyncTriggerBar.animate().scaleX(0f)
	...
	// If sync is turned off on this device or account, remind the user with a
	// toast.
	// private void showToastIfSyncIsOff() {
	// final int reasonSyncOff =
	// ConversationSyncDisabledTipView.calculateReasonSyncOff(
	...
	// if (num > 0 && num <= MAX_NUM_OF_SYNC_TOASTS) {
	// Toast.makeText(getContext(), R.string.account_sync_off,
	// Toast.LENGTH_SHORT)
	// .show();
	...
	public void onSyncFinished() {
		// onSyncFinished() can get called several times as result of folder
		// updates that maybe
		...
		if (mIsSyncing) {
			...
			mSyncTriggerBar.setVisibility(GONE);
			// Hide the "Checking for mail" text in action bar if it isn't
			// hidden already:
			mHintText.hide();
			mIsSyncing = false;
		}
	...
	private WindowManager.LayoutParams getRefreshHintTextLayoutParams() {
		// Create the "Swipe down to refresh" text view that covers the action
		// bar.
		Rect rect = new Rect();
	...
	protected static class HintText extends FrameLayout {
		...
		public static int getActionBarBackgroundResource(final Context context) {
			...
			if (context.getTheme().resolveAttribute(
				...
				final TypedArray attr = context.obtainStyledAttributes(
						actionBarStyle.resourceId, STYLE_ATTR);
		...
		private void displayCheckingForMailAndHideAfterDelay() {
			...
			postDelayed(mHideHintTextRunnable,
		...
		private void hide() {
			if (mDisplay != NONE) {
				...
				postDelayed(mSetVisibilityGoneRunnable,
						SWIPE_TEXT_APPEAR_DURATION_IN_MILLIS);
				mDisplay = NONE;
			}
		}
